...
- `test_abstract`: describes the test operation that will be executed
...
The `terraform test` command will emit a `test_abstract` message describing the test files and `run` blocks that will be executed during the upcoming operation.
...
After each test file has completed execution, `terraform test` will emit a `test_file` message with the status of the test file.
...
After each test file has completed execution, `terraform test` will emit a series of `test_run` messages with the status of each run block within that test file.
...
After each test file has completed execution, `terraform test` may emit zero to many`test_cleanup` message with any state that the test framework could not clean up. The resources listed in these messages will need to be cleaned up manually.

As the test framework can manage multiple state files for a given test file you may see multiple versions of this message. One for each state file that could not be cleaned up. You can pinpoint the run block that last updated this particular state, so you can detect where the dangling resources are located, by looking at the `@testrun` field.
...
After the test operation has completed all test files, `terraform test` will emit a `test_summary` message with the status of the overall test operation.
...
In `-verbose` mode, `terraform test` will emit a `test_plan` message for all `run` blocks that executed a plan operation.  The `test_plan` message content matches the format of the `terraform show -json <planfile>` command [output](/terraform/internals/json-format#plan-representation).
...
```json
{
  ...
  "test_plan": {
    ...
    "variables": {
      "input": {
        "value": "D208E38B-9F69-4A18-8839-D34869021AC4"
      }
    },
    "planned_values": {
      "root_module": {
        "resources": [
          {
            ...
            "schema_version": 0,
            "values": {
              "ami": "D208E38B-9F69-4A18-8839-D34869021AC4"
            },
            "sensitive_values": {}
          }
        ]
    ...
    "timestamp": "2023-08-09T15:10:05Z",
    "errored": false
  },
  "type": "test_plan"
}
...
In `-verbose` mode, `terraform test` will emit a `test_state` message for all `run` blocks that executed an apply operation. The `test_state` message content matches the format of the `terraform show -json` command [output](/terraform/internals/json-format#state-representation).
...
When `terraform test` is interrupted or cancelled it will emit a `test_interrupt` message that describes the operation that was being completed when it was interrupted. This will include any state that could not be cleaned up and any resources that were being created.

As with the [Test Cleanup](#test-cleanup) message, there is the potential for multiple state files to have been left dangling. The resources from the main state for the configuration under test is contained in the `state` field, while the `states` field contains a map from `run` block names to resources created by that `run` block that could not be cleaned up.

The `planned` field contains any resources that were in the process of being created by the `run` block that was interrupted, which is identified by the `@testrun` field.
